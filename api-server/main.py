#!/usr/bin/env python3
"""JARVIS Cloud — FastAPI backend with PostgreSQL/SQLite state (ported from Agentic OS)."""
...
# Supports both PostgreSQL (via DATABASE_URL) and SQLite (via DATA_DIR).
...
def get_db():
    """Return a DB adapter — PostgreSQL if DATABASE_URL is set, else SQLite."""
    if DATABASE_URL:
        import psycopg2
...
class _SQLiteConnection:
    """Wraps a standard sqlite3 connection identically."""
...
# ─── PostgreSQL adapter — translates sqlite3 patterns ────────────────

class _PGConnection:
    """Wraps a psycopg2 connection, translating sqlite-style SQL to PG."""
    def __init__(self, conn):
        self._conn = conn

    def _translate(self, sql):
        """Convert sqlite3 SQL idioms to PostgreSQL equivalents."""
        sql = sql.replace("?", "%s")
        sql = sql.replace("datetime('now')", "NOW()")
        sql = sql.replace("AUTOINCREMENT", "GENERATED BY DEFAULT AS IDENTITY")
        # Remove PRAGMA statements (not applicable to PG)
        lines = []
    ...
    def executescript(self, sql):
        """Run multi-statement SQL, splitting by semicolons."""
...
class _PGResult:
    """Mimics sqlite3 cursor results — .fetchone(), .fetchall()."""
    ...
    def fetchone(self):
        ...
        if row is None:
            return None
        return _PGRow(row, self._description)

    def fetchall(self):
        rows = self._cursor.fetchall()
        if rows is None:
            return []
        return [_PGRow(r, self._description) for r in rows]


class _PGRow:
    """Mimics sqlite3.Row — supports dict(r), r['col'], r[0], len(r), keys()."""
    ...
    def __getitem__(self, key):
        if isinstance(key, str):
            idx = self._columns.index(key.lower() if key.islower() else key)
...
app = FastAPI(title="JARVIS Cloud — Agentic OS", version="3.1")

...
1. **Use `--max-turns`** in print mode to prevent runaway loops. Start with 5-10 for most tasks. For large single-file HTML/CSS/JS rebuilds (3000+ lines), use 40-50 turns — 30 turns is often insufficient for comprehensive rewrites with multiple feature additions.
...
- `references/agentic-os-boot.md` — Boot procedure for Elias's Agentic OS (Hermes + Claude Code + MT5 + Obsidian). Use when user says "run the agentic os" or similar.
- `references/multi-agent-ceo-pattern.md` — Hermes as CEO orchestrating Claude Code + Codex in parallel for large builds (websites, startup packages). Covers CLAUDE.md spec writing, agent deployment, verification, vault sync.

## Rules for Hermes Agents

1. **Prefer print mode (`-p`) for single tasks** — cleaner, no dialog handling, structured output
2. **Use `--allowedTools` over `--dangerously-skip-permissions` in print mode** — `--allowedTools "Read,Write,Edit,Bash"` grants full file access without triggering the permissions dialog. `--dangerously-skip-permissions` may be blocked by the terminal approval system.
